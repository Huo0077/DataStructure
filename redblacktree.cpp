...
//
//   【情况 2】叔叔 U 是黑色，且 N 与 P 在不同侧（折线型：N 是 P 的右子）
//            G [B]                           G [B]
//           /     \        对 P 左旋         /     \
//        P [R]   U [B]     ─────────>     N [R]   U [B]   (转化为情况 3)
//          \                              /
//          N [R]                        P [R]
//
//   【情况 3】叔叔 U 是黑色，且 N 与 P 在同一侧（直线型：N 是 P 的左子）
//            G [B]                           P [B]
//           /     \       P染黑, G染红       /     \
//        P [R]   U [B]    ───────────>    N [R]   G [R]  (修复完成!)
//        /                 对 G 右旋               \
//      N [R]                                      U [B]
//
...
// 【图示约定与阅读地图】
//   B / R 表示节点颜色，NIL 哨兵按黑色叶节点理解。阅读修复代码时先数黑高，再看局部颜色冲突。
//   红黑树不追求每层绝对平衡；它维护“红节点不相邻”和“任一路径黑高一致”，以约束树高。
//   插入修复从红红冲突出发，删除修复从额外黑色出发，两者的循环不变量不同，不应混读。
...
private:
    ...
    //          P [B]                                  S [B]
    //         /     \         P 染红, S 染黑          /     \
    //      x [BB]   S [R]     ────────────>        P [R]   S->right [B]
    //              /     \     对 P 左单旋         /     \
    //           S->left S->right                x [BB] S->left
    //      → 转化为情况 2/3/4 (此时 x 的新兄弟必为黑色)
    //
    ...
    //                    /   \                                      /   \
    //                  [B]   [B]                                  [B]   [B]
    //      → 双重黑色推给父节点 P。若 P 原本为红，下轮循环终止染黑即可；若 P 为黑，继续向上回溯。
    //
    //   【情况 3】兄弟 S 是黑色，且 S 的 近侧（左子）为红，远侧（右子）为黑
    //          P (颜色任意)                            P (颜色任意)
    //         /          \       近侄染黑, S 染红     /          \
    //      x [BB]        S [B]   ───────────────>   x [BB]     S->left [B]
    //                    /   \     对 S 右单旋                     \
    //                 [R]     [B]                                  S [R]
    //                (近侄)  (远侄)                                 \
    //                                                               [B]
    //      → 转化为情况 4 (红色转移到远侧)
    //
    //   【情况 4】兄弟 S 是黑色，且 S 的 远侧（右子）为红色（绝杀！）
    //          P (颜色 C)                               S (继承颜色 C)
    //         /          \       S 继承 P 颜色         /             \
    //      x [BB]        S [B]   P 与 远侄 染黑      P [B]           远侄 [B]
    //                    /   \   ─────────────>     /     \
    //                  任意  [R]   对 P 左单旋    x [B]   任意
    //                       (远侄)
    //      → 一次旋转变色彻底消除双重黑色，全树平衡，算法结束！
    // ----------------------------------------------------------
