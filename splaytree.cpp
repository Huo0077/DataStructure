...
void demoZigZig()
{
    ...
    // 构造: G=3 (根), P=2 (左), X=1 (左左)
    SplayNode* G = new SplayNode(3);
    SplayNode* P = new SplayNode(2);
    SplayNode* X = new SplayNode(1);
    SplayNode* D = new SplayNode(4);   // G 的右子
    SplayNode* C = new SplayNode(2.5); // P 的右子(但不好表示，用一个接近的值)
    // 简化一下构造，只用整数
    // 重新构造: 根=5, 左子=3, 左左=1, 左右=4, 左左右=2,
    // 我们演示一个更简单的 Zig-Zig
